		...
		do
			inspect
				...
				when 3 then
					--SPECIAL
					regen := regen + 10
					model.m.enemy_act_display_str.append ("    A Carrier(id:"+id.out+") gains 10 regen.%N")
				else
				end
		end
...
move_enemy(steps:INTEGER)
		...
		if location.column >=1  then
			...
			if location.column >=1  then
				model.m.enemy_act_display_str.append ("    A Carrier(id:"+id.out+") moves: ["+model.m.row_indexes.item (location.row).out+","+(old_location.column).out+"] -> ["+model.m.row_indexes.item (location.row).out+","+location.column.out+"]%N")
			end
