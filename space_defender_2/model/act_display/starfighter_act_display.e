	...
	display_act(type: INTEGER):STRING
		do
			...
			--CHANGE FOR SPREAD AND ROCKET
				...
				Result.append ("      A friendly projectile(id:"+model.m.projectile_id.out+") spawns at location ["+model.m.row_indexes.item (model.m.ship.location.row).out+","+(model.m.ship.location.column+1).out+"].")
